...
# # ------------------------------------------------------------------------------------------------------------------------------------
# # comparison (to be continued tomorrow)
# # ------------------------------------------------------------------------------------------------------------------------------------
# # Only with G17
# obs.G17.ccl <- OBS_ccl1(y1, F)
# start_time <- Sys.time()
# fit.G17 <- Gibbs_FFBS1(obs.G17.ccl, G, m0, C0, N.sample)
# save(fit.G17, file=here::here("results", "fit.G17.RData"))
# end_time <- Sys.time()
# load(here::here("results", "fit.G17.RData"))  
# 
# # plot the filtering results only with G17
# p.G17.aod.flt <- list()
# for(i in 1:20){
#   tempt <- data.frame(long=G17.aod.slt[[1]]$long, lat=G17.aod.slt[[1]]$lat, AOD = F%*%fit.G17$m.flt[[i+1]][1:N^2])
#   p.G17.aod.flt[[i]] <- 
#     ggplot(tempt, aes(long, lat, fill=AOD)) +
#     geom_raster() + 
#     scale_fill_viridis_c(option = "D", limits = c(NA,3.5)) + 
...
#     coord_cartesian(xlim=c(-124,-121.6),ylim=c(35.0,37.4)) +
#     ggtitle(paste(c("time", as.character(i)), collapse=" "))
#   print(p.G17.aod.flt[i])
# }
# 
# # Only with G16
# G16.aod.slt <- UDS(G16.aod.raw, 22)
# y2 <- list()
# for (i in 1:20){
#   y2[[i]] <- G16.aod.slt[[i]]$AOD
# }
...
# start_time <- Sys.time()
# fit.G16 <- Gibbs_FFBS1(obs.G16.ccl, G, m0, C0, N.sample)
# save(fit.G16, file=here::here("results", "fit.G16.RData"))
# end_time <- Sys.time()
# load(here::here("results", "fit.G16.RData"))  
# 
# # plot the filtering results only with G16
# p.G16.aod.flt <- list()
# for(i in 1:20){
#   tempt <- data.frame(long=G16.aod.slt[[1]]$long, lat=G16.aod.slt[[1]]$lat, AOD = F%*%fit.G16$m.flt[[i+1]][1:N^2])
#   p.G16.aod.flt[[i]] <- 
#     ggplot(tempt, aes(long, lat, fill=AOD)) +
#     geom_raster() + 
#     scale_fill_viridis_c(option = "D", limits = c(NA,3.5)) +
